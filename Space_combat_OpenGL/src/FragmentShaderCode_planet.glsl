...
vec3 CalcDirLight(DirLight dirLight)
{
	...
	float brightness = dot(lightDir, normalize(normal));
...
{
	//texture
	vec3 textures = (texture(myTextureSampler0, UV) + 0.5 * texture(myTextureSampler2, UV) + texture(myTextureSampler3, UV)).rgb;
	normal = texture(myTextureSampler1, UV).rgb;
	normal = normalize(normal * 2.0 - 1.0);

	vec3 finalColor = (lightIntensity * CalcDirLight(dirLight) + 2.0 * CalcPointLight(pointLight)) * textures;
